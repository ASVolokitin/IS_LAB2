...
export const CreateCoordinatesPage = () => {
  ...
  const handleSubmit = useCallback((dto: CoordinatesDTO) => {
        setServerError(null);
        setServerStatus(null); // Очищаем статус/ошибку перед новым запросом
        
        createCoordinates(dto)
            .then(() => { 
                setServerStatus(`Successfully created coordinates`); 
                navigate(-1); 
            })
            .catch((err) => {
    ...
    }, [navigate]); // ⬅️ Зависимости: только navigate

    // 2. Стабилизируем onCancel
    ...
    <>
      ...
      <div className="create-object-page">
        <div className="form-container">
          ...
          <CoordinatesForm onSubmit={handleSubmit} onCancel={handleCancel}  />
          <p>{serverStatus}</p>
